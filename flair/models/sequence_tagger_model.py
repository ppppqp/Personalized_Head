...
class SequenceTagger(flair.nn.Classifier):
    ...
    def _obtain_labels(
            ...
            get_all_tags: bool, ) -> (List[List[Label]], List[List[List[Label]]]):
        """
        ...
        lengths: List[int] = [len(sentence.tokens)
    ...
    def _viterbi_decode(
        ...
        backpointers = np.empty(
            shape=(feats.shape[0], self.tagset_size), dtype=np.int_)
        backscores = np.empty(
    ...
    def _fetch_model(model_name) -> str:
        ...
        hu_model_map = {
            ...
            "hunflair-paper-cellline": "/".join(
                [hu_path, "hunflair_smallish_models",
                    "cellline", "hunflair-celline-v1.0.pt"]
            ),
            "hunflair-paper-chemical": "/".join(
                [hu_path, "hunflair_smallish_models",
                    "chemical", "hunflair-chemical-v1.0.pt"]
            ),
            "hunflair-paper-disease": "/".join(
                [hu_path, "hunflair_smallish_models",
                    "disease", "hunflair-disease-v1.0.pt"]
            ),
            "hunflair-paper-gene": "/".join(
                [hu_path, "hunflair_smallish_models",
                    "gene", "hunflair-gene-v1.0.pt"]
            ),
            "hunflair-paper-species": "/".join(
                [hu_path, "hunflair_smallish_models",
                    "species", "hunflair-species-v1.0.pt"]
            ),
            ...
            "hunflair-chemical": "/".join(
                [hu_path, "hunflair_allcorpus_models",
                    "huner-chemical", "hunflair-chemical-full-v1.0.pt"]
            ),
            "hunflair-disease": "/".join(
                [hu_path, "hunflair_allcorpus_models",
                    "huner-disease", "hunflair-disease-full-v1.0.pt"]
            ),
            "hunflair-gene": "/".join(
                [hu_path, "hunflair_allcorpus_models",
                    "huner-gene", "hunflair-gene-full-v1.0.pt"]
            ),
            "hunflair-species": "/".join(
                [hu_path, "hunflair_allcorpus_models",
                    "huner-species", "hunflair-species-full-v1.1.pt"]
            )}
        ...
        # if not, check if model key is remapped to direct download location. If so, download model
            ...
            model_path = cached_path(
                hu_model_map[model_name], cache_dir=cache_dir)
        ...
        # special handling for the taggers by the @redewiegergabe project (TODO: move to model hub)
            ...
            if not model_file.exists():
                ...
                unzip_file(flair.cache_root / cache_dir /
                           'indirect.zip', flair.cache_root / cache_dir)
            model_path = str(flair.cache_root / cache_dir /
                             'indirect' / 'final-model.pt')
            ...
            if not model_file.exists():
                ...
                unzip_file(flair.cache_root / cache_dir /
                           'direct.zip', flair.cache_root / cache_dir)
            model_path = str(flair.cache_root / cache_dir /
                             'direct' / 'final-model.pt')
            ...
            if not model_file.exists():
                ...
                unzip_file(flair.cache_root / cache_dir /
                           'reported.zip', flair.cache_root / cache_dir)
            model_path = str(flair.cache_root / cache_dir /
                             'reported' / 'final-model.pt')

        elif model_name == "de-historic-free-indirect":
            model_file = flair.cache_root / cache_dir / 'freeIndirect' / 'final-model.pt'
            if not model_file.exists():
                cached_path(
                    'http://www.redewiedergabe.de/models/freeIndirect.zip', cache_dir=cache_dir)
                unzip_file(flair.cache_root / cache_dir /
                           'freeIndirect.zip', flair.cache_root / cache_dir)
            model_path = str(flair.cache_root / cache_dir /
        ...
        if get_from_model_hub:
            ...
            url = hf_hub_url(model_name, revision=revision,
            ...
            try:
                ...
                log.error("-" * 80)
                # remove folder again if not valid
