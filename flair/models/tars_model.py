...
class FewshotClassifier(flair.nn.Classifier):
    ...
    def _get_tars_formatted_sentences(self, sentences: List[Sentence]):
        label_text_pairs = []
        all_labels = [label.decode(
            "utf-8") for label in self.get_current_label_dictionary().idx2item]
        # print(all_labels)
        for sentence in sentences:
            ...
            if self.training and self.num_negative_labels_to_sample is not None:
                ...
                sampled_negative_labels = self._get_nearest_labels_for(
                    positive_labels)
    ...
    def _get_nearest_labels_for(self, labels):
        ...
        for label in labels:
            ...
            plausible_label_probabilities /= np.sum(
                plausible_label_probabilities)
    ...
    def _compute_label_similarity_for_current_epoch(self):
        ...
        if isinstance(self.tars_embeddings, TokenEmbeddings):
            encodings_np = [sentence[0].get_embedding().cpu().detach().numpy()
                            for sentence in label_sentences]
        else:
            encodings_np = [sentence.get_embedding().cpu().detach().numpy()
                            for sentence in label_sentences]
...
class TARSTagger(FewshotClassifier):
    ...
    def __init__(
        ...
        if task_name and label_dictionary and label_type:
            ...
            self.add_and_switch_to_new_task(
                task_name, label_dictionary, label_type)
        else:
    ...
    def _get_tars_formatted_sentence(self, label, sentence):
        ...
        for token in sentence:
            ...
            tars_sentence.get_token(
                token.idx + label_length).add_tag(self.static_label_type, tars_tag)
    ...
    def predict(
        ...
        dataloader = DataLoader(dataset=SentenceDataset(
            reordered_sentences), batch_size=mini_batch_size)
...
class TARSClassifier(FewshotClassifier):
    ...
    def __init__(
            ...
            label_dictionary: Optional[Dictionary] = None,
            label_type: Optional[str] = None,
            # 'google/mobilebert-uncased', #"bert-base-uncased",#'prajjwal1/bert-tiny',#
            embeddings: str = "bert-base-uncased",
            num_negative_labels_to_sample: int = 2,
            prefix: bool = True,
        ...
        embeddings.fine_tune = fine_tune
        # initialize a bare-bones sequence tagger
    ...
    def _get_tars_formatted_sentence(self, label, sentence):
        ...
        sentence_labels = [self._clean(label.value) for label in sentence.get_labels(
            self.get_current_label_type())]
    ...
    def _fetch_model(model_name) -> str:
        ...
        model_map["tars-base"] = "/".join([hu_path,
                                          "tars-base", "tars-base-v8.pt"])
                    ...
                    if not multi_label:
                        if len(sentence.get_labels(label_name)) > 0:
                            ...
                            label_scores = torch.tensor([label.score for label in sentence.get_labels(label_name)],
                                                        dtype=torch.float)
                            best_label = sentence.get_labels(
                                label_name)[torch.argmax(label_scores)]

                            # remove previously added labels and only add the best label
                            sentence.remove_labels(label_name)
                            sentence.add_label(
                                typename=label_name, value=best_label.value, score=best_label.score)
