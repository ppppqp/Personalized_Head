...
class TextClassifier(flair.nn.DefaultClassifier):
    ...
    def forward_pass(self,
        ...
        text_embedding_list = [sentence.get_embedding(embedding_names).unsqueeze(0).unsqueeze(1) for sentence in sentences]#add .unsqueeze(1)
        
        text_embedding_tensor = torch.cat(text_embedding_list, 0).to(flair.device)
        #adding a extra encoder layer
        encoder_tensor = self.encoder(text_embedding_tensor)
