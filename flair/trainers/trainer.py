...
def count_parameters(model):
    ...
    for name, parameter in model.named_parameters():
        ...
        total_params+=param
    # print(table)
    print(f"Total Trainable Params: {total_params-109482242}")
...
class ModelTrainer:
    ...
    def train(
        ...
        #print(f"\n\n Training parameter : {model_card['training_parameters']}\n\n")
        ...
        for name, parameter in self.model.named_modules():
                if name=="tars_model.encoder":
                    log.info(f'HEAD: "{self.model.tars_model.encoder}"')
        log_line(log)
        ...
        try:
            ...
            for epoch in range(epoch + 1, max_epochs + 1):
                ...
                if log_train:
                    ...
                    log.info(
                        f"Train : loss {train_eval_result.loss} - {main_evaluation_metric[1]} ({main_evaluation_metric[0]})  {round(train_eval_result.main_score, 4)}"
                    )
            ...
            if save_final_model and not param_selection_mode:
                for name, parameter in self.model.named_modules():
                    if name=="tars_model.encoder":
                        torch.save(self.model.tars_model.encoder.state_dict(),base_path / "encoder.pt")
                self.model.save(base_path / "final-model.pt", checkpoint=save_optimizer_state)
                # quant_model=torch.quantization.quantize_dynamic(
                # self.model.to(device='cpu'),  # the original model
                # {torch.nn.Linear},  # a set of layers to dynamically quantize
                # dtype=torch.qint8)
                # quant_model.save(base_path / "quant-model.pt")
                # for name, parameter in quant_model.named_modules():
                #     if name=="tars_model.encoder_layer":
                #         torch.save(quant_model.tars_model.encoder_layer.state_dict(),base_path / "enc_quant.pt")
    ...
    def final_test(
        ...
        # elif(base_path / "quant-model.pt").exists() :
        #     log.info("Testing using Quant model ...")
        #     self.model.load_state_dict(self.model.load(base_path / "quant-model.pt").state_dict())
        else:
