...
def split_balanced(data, target, test_size=0.1):
    ...
    for cl in classes:
        if (n_train_per_class+n_test_per_class) > np.sum(target == cl):
            ...
            splitix = int(np.ceil(
                n_train_per_class/(n_train_per_class+n_test_per_class)*np.sum(target == cl)))
...
train_ds = []
test_ds = []
train_text, train_label, test_text, test_label = split_balanced(
    text_data, index_list)

for datapoint, class_val in zip(train_text, train_label):
    train_ds.append(Sentence(datapoint.lower()).add_label(
        'snips_mobile_data', intent_list[class_val].lower()))
...
def train_module(model, fine_tune, ff_dim, nhead):
    ...
    if model == "BERT":
        ...
        tars = TARSClassifier.load(
            "tars-base", nhead=nhead, ff_dim=ff_dim, fine_tune=fine_tune)
    print(f"\n\nTime taken to load the model : {time.time()-start_time}\n\n")

    tars.add_and_switch_to_new_task("snips_mobile_data", label_dictionary=corpus.make_label_dictionary(
        label_type="snips_mobile_data"), label_type="snips_mobile_data")

    trainer = ModelTrainer(tars, corpus)

    start_time = time.time()

    data = trainer.train(base_path=f'taggers/snips_full_{model}',  # path to store the model artifacts
                         learning_rate=0.02,
                         mini_batch_size=16,
                         max_epochs=100,
